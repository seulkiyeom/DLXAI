...
def visualize(relevances, images_tensor=None, name=''):
	...
	for h,heat in enumerate(relevances):
		if images_tensor is not None:
			input_image = images_tensor[h]
			for tz in xrange(z):
				for tdim in xrange(dim):
					plt.imshow(heat[:,:,tz,tdim], interpolation='nearest', norm=ml.colors.Normalize(vmin=-0.00001, vmax=0.00001))
					#plt.imshow(heat[:,:,tz,tdim])
					plt.colorbar(orientation='vertical')
					#plt.show()
					plt.savefig('./eeg_res2/'+str(h)+'_depth_'+str(tdim)+'_time_'+str(tz)+'.png')
					plt.clf()
				'''print(heat[:,:,z,:])
				maps = render.hm_to_rgb(in_heat, input_image, scaling = 3, shape = [],sigma = 2)'''
		else:
			maps = render.hm_to_rgb(heat, scaling = 3, sigma = 2)
		#heatmaps.append(maps)
	#R = np.array(heatmaps)
