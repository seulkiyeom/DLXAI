...
class AvgPool(Module):
	...
	def forward(self,input_tensor, batch_size=10, img_dim=28):
		...
		self.in_N, self.input_x_dim, self.input_y_dim, self.input_z_dim, self.in_depth = self.input_tensor.get_shape().as_list()
		
		
		with tf.name_scope(self.name):
			self.activations = tf.nn.avg_pool3d(self.input_tensor, ksize=self.pool_kernel,strides=self.pool_stride,padding=self.pad, name=self.name )
	...
	def extract_patches(self):
		...
		for z in range(0, dim[2]/self.stride_size):
			for y in range(0, dim[1]/self.stride_size):
				for x in range(0, dim[0]/self.stride_size):
					conv_indexes.extend([[[x, y, z], (list([x*self.stride_size + i, y*self.stride_size + j, z*self.stride_size + k]
					for i in range(0, self.pool_size)
					for j in range(0, self.pool_size)
					for k in range(0, self.pool_size)))]])
		...
		for nth_layer in range(0, self.batch_size):
			for nth_depth in range(0, self.input_depth):
				for patches in conv_indexes:
					patch = []
					for index in patches[1]:
						print(index, nth_layer, nth_depth)
						patch.append(input_tensor[nth_layer][index[0]][index[1]][index[2]][nth_depth])
					output[nth_layer][patches[0][0]][patches[0][1]][patches[0][2]][nth_depth] = np.array(patch)
	
		return tf.reshape(output, [self.in_N, dim[0]/self.stride_size, dim[1]/self.stride_size, dim[2]/self.stride_size, self.pool_size, self.pool_size, self.pool_size, self.input_depth])


	def compute_z(self, image_patches):

		return tf.multiply(tf.expand_dims(self.weights, 0), tf.expand_dims(tf.cast(image_patches, tf.float32), -1))


	def compute_zs(self, Z, stabilizer = True, epsilon = 1e-12):

		Zs = tf.reduce_sum(Z, [4, 5, 6, 7], keep_dims=True)
		if stabilizer==True:
	...
	def compute_result(self, Z, Zs):
		self.Xout = (self.input_x_dim + self.pool_size)/self.stride_size
		self.Yout = (self.input_y_dim + self.pool_size)/self.stride_size
		self.Zout = (self.input_z_dim + self.pool_size)/self.stride_size
		result = tf.reduce_sum((Z/Zs) * tf.reshape(self.R, [self.in_N, self.Xout, self.Yout, self.Zout, 1, 1, 1, 1, self.output_depth]), 6)

		return tf.reshape(result, [self.in_N,self.Xout,self.Yout, self.Zout, self.pool_size*self.pool_size*self.pool_size*self.input_depth])


	def restitch_image(self, result):

		dim = [self.input_x_dim, self.input_y_dim, self.input_z_dim]
		...
		output = np.zeros([self.in_N, dim[0], dim[1], dim[2], self.output_depth], dtype=object)
		for nth_layer in range(0, self.batch_size):
			for nth_depth in range(0, self.output_depth):
				for patches in conv_indexes:
					patch = result[patches[0][0]][patches[0][1]][patches[0][2]]
					for index in patches[1]:
						output[nth_layer][index[0]][index[1]][index[2]][nth_depth] += patch
