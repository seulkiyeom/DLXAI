...
class Convolution(Module):
	...
	def __init__(self, output_depth, batch_size=None, input_x_dim=None, input_y_dim=None, input_z_dim=None, input_depth=None, kernel_size=5, stride_size=2, act = 'linear', keep_prob=1.0, pad = 'SAME', weights_init= tf.truncated_normal_initializer(stddev=0.01), bias_init= tf.constant_initializer(0.0), name="conv3d"):
		self.name = name
		#self.input_tensor = input_tensor
		Module.__init__(self) #layer naming
		
		
		self.batch_size = batch_size #한번에 학습할 데이터량
		self.input_x_dim = input_x_dim #input영상의 x
		self.input_y_dim = input_y_dim  # input영상의 y
		self.input_z_dim = input_z_dim  # input영상의 z (여기서는 time)
		self.input_depth = input_depth #RGB 차원(?) - 여기서는 흑백이니까 1
		

		self.output_depth = output_depth #Filter 수와 일치
		self.kernel_size = kernel_size #Filter size
		self.stride_size = stride_size
		self.act = act #Activation function 이름 (relu, tanh 등등)
		self.keep_prob = keep_prob
		self.pad = pad #padding 방법
	...
	def check_input_shape(self):
		...
		try:
			if len(inp_shape)!=5:
				mod_shape = [self.batch_size, self.input_x_dim,self.input_y_dim,self.input_z_dim,self.input_depth]
				self.input_tensor = tf.reshape(self.input_tensor, mod_shape)
		except:
			raise ValueError('Expected dimension of input tensor: 5')
	...
	def forward(self, input_tensor):
		...
		#in_N: input trial 수, in_h: input 세로, in_w: input 가로, in_depth: input 필터수(처음은 RGB영상의 경우 3)

		# init weights
		self.weights_shape = [self.kernel_size, self.kernel_size, self.kernel_size, self.input_depth, self.output_depth]
	...
	def _simple_lrp(self,R):
		...
		xf,yf,zf,df,NF = self.weights_shape
	...
	def _ww_lrp(self,R): 
		...
		image_patches = tf.ones([self.in_N, self.Hout,self.Wout, self.kernel_size,self.kernel_size, self.input_depth])
		#pdb.set_trace()
		ww = tf.square(self.weights)
		Z = tf.expand_dims(ww,0)
		#self.Z = tf.expand_dims(tf.tile(tf.reshape(ww, [1,1,self.kernel_size, self.kernel_size, self.input_depth, self.output_depth]), [self.Hout, self.Wout, 1,1,1,1]), 0)
	...
	def _flat_lrp(self,R):
		...
		Z = tf.ones([self.in_N, self.Hout,self.Wout, self.kernel_size,self.kernel_size, self.input_depth, self.output_depth])
	...
	def check_shape(self, R):
		...
		try:
			...
			activations_shape = self.activations.shape
